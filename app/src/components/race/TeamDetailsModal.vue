...
function formatValue(v) {
  ...
  if (Array.isArray(v)) {
    var parts = v.filter(function (x) { return !isEmptyValue(x); });
  ...
  if (typeof v === "object") {
    ...
    Object.keys(v).forEach(function (k) {
      ...
      if (Array.isArray(vv)) {
        var filtered = vv.filter(function (x) { return !isEmptyValue(x); });
...
function collectFields(obj, defs) {
  var out = [];
  if (!obj) return out;
